	...
	public static void main(String [] args) {
		...
		for (int numToCheck = start; numToCheck < numSteps*increment + start; numToCheck += increment)
		{
			...
			for (int i = 0; i < trials; i++) {
				BasicDocument basicDoc = new BasicDocument(readText);
				fleschScore = basicDoc.getFleschScore();
			}
			endTime = System.nanoTime();
			double durationTime = (endTime - startTime) / 1000000000.0;
			System.out.printf(durationTime + "\t");
			startTime = System.nanoTime();
			for (int i = 0; i < trials; i++) {
				EfficientDocument efficientDoc = new EfficientDocument(readText);
				fleschScore = efficientDoc.getFleschScore();
			}
			endTime = System.nanoTime();
			durationTime = (endTime - startTime) / 1000000000.0;
			System.out.printf(durationTime + "\n");
